...
async def send_navigation_goal(x: float, y: float, ctx: Context) -> str:
    ...
    await ctx.info('Waiting for Nav2 to become active...')
    navigator.waitUntilNav2Active()
    await ctx.info('Nav2 is active')
    ...
    while not navigator.isTaskComplete():
        ...
        if feedback and i % 5 == 0:
            ...
            await ctx.info(f'Estimated time of arrival: {seconds:.0f} seconds.')
    ...
    if result == TaskResult.SUCCEEDED:
        ...
        navigator.get_logger().warn(f'Unexpected navigation result: {result}')
